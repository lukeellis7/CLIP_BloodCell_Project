...
def run_evaluation(pred_csv: str,
    ...
    # Make sure types are strings (avoids float/NaN label sort issues)
    ...
    cm_path = os.path.join(
        results_dir,
        "confusion_matrix_norm.png" if normalize_cm else "confusion_matrix.png"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--normalize_cm", action="store_true",
                        help="If set, plot a row-normalized (percentage) confusion matrix")
    args = parser.parse_args()
    run_evaluation(args.pred_csv, args.results_dir, normalize_cm=args.normalize_cm)
